...
echo -e "${ORANGE}Overwriting the conf.mk to fix the ram issue${NC}"
cp ../../dts/optee-os/conf.mk core/arch/arm/plat-stm32mp1/
# Perform cleanup
echo -e "${GREEN}Perform cleanup${NC}"
make CROSS_COMPILE=${CC} clean
# Perform build for usb boot
echo -e "${GREEN}Build it${NC}"
make CROSS_COMPILE=${CC} PLATFORM=stm32mp1 CFG_EMBED_DTB_SOURCE_FILE=stm32mp151a-tios-mx.dts CFG_TEE_CORE_LOG_LEVEL=2 O=build all
...
make CROSS_COMPILE=${CC} PLATFORM=stm32mp1 CFG_EMBED_DTB_SOURCE_FILE=stm32mp151a-tios-mx.dts CFG_TEE_CORE_LOG_LEVEL=4 DEBUG=1 O=build all
